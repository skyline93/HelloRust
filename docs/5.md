...
函数以`fn`关键字开头，函数名使用`蛇形命名法`（只使用小写字母，以下划线分隔单词）。
...
函数参数分为参数变量(parameter)和传入的具体参数(argument)，参数变量必须显示声明数据类型；多个参数可以是不同的类型，使用逗号隔开。
...
fn main() {
  ...
  let y = {     // 括号括起来的为代码块
    let x = 3;  // 语句
    x + 1       // 表达式，此处末尾如果加入了分号，则这个代码块不返回值
  };
